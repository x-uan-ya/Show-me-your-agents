...
export function ImportData({
  ...
  const doUpload = async () => {
    ...
    try {
      const response = await api.uploadDataset(clientId, file, undefined, controller.signal);
      setUpload(response);
      ...
      <div className="mx-auto max-w-6xl space-y-6">
        ...
        <section className="surface-card p-5 sm:p-6">
          <div className="mb-5 flex items-start justify-between gap-4">
            <div>
            <p className="import-step-label">Step 1</p>
              <h2 className="mt-1 text-lg font-semibold text-white">
                Choose the client context
              </h2>
            </div>
            <span className="context-pill">
              {clientId === null ? "Required" : `Client #${clientName || "selected"}`}
            </span>
          </div>
          <ClientSelector
            selectedId={clientId}
        ...
        <section className="surface-card p-5 sm:p-6">
          <div className="mb-4">
            <p className="import-step-label">Step 2</p>
            <h2 className="mt-1 text-lg font-semibold text-white">
              Upload a CSV file
            </h2>
            <p className="mt-1 text-sm text-slate-400">
        ...
        {upload && (
          <section className="surface-card space-y-6 p-5 sm:p-6">
            <div>
              <p className="import-step-label">Step 3</p>
        ...
        {error && (
          <p
            role="alert"
            className="rounded-xl border border-amber-700 bg-amber-950/40 p-4 text-sm text-amber-200"
          >
            {error}
          </p>
        )}
